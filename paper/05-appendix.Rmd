...
Where $y_{ij}^{+}$ (with $\diamond_j=+$) and $y_{ij}^{-}$ (with $\diamond_j=-$) refer to evaluation as right limits and left limits, respectively. For $d=2$ the innermost sum is over the set $(\diamond_1,\diamond_2) \in \{-,+\}^2$, i.e. $\{(\diamond_1=-,\diamond_2=-),(\diamond_1=-,\diamond_2=+),(\diamond_1=+,\diamond_2=-),(\diamond_1=+,\diamond_2=+) \}$. So the inner sum $Q$ is: 
\begin{eqnarray}
Q & = &(-1)^{2} C_{\theta}(F_1(y_{i1}^{-};\mathbf{\gamma}_1),F_d(y_{i2}^{-};\mathbf{\gamma}_2)) + (-1)^{1} C_{\theta}(F_1(y_{i1}^{-};\mathbf{\gamma}_1),F_d(y_{i2}^{+};\mathbf{\gamma}_2)) + \nonumber \\ 
