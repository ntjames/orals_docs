...
The margins can be extended to depend on covariates by replacing each marginal df with a conditional df tied to a regression model. $F_j(Y_j)$ is replaced by $F_j(Y_j|X_j)$ where $X_j$ is a vector of $p_j$ known covariates for the $j^{th}$ marginal regression model and $x_j$ is an observed value of the $j^{th}$ vector of covariates. In a clinical trial the primary covariate of interest is usually treatment group or dose level and these covariates would be common to all margins, however the $j$ marginal models are not required to have a common set of covariates or to use the same functional form for covariates.  For each $j=1,\ldots,d$ there is a *marginal calibration function* $\varphi_j$ which relates the observed $\mathbf{x}_j$ to the parameters of $F_j$. One possible marginal calibration function for the location parameter of $F_j$ is $\gamma_{j,\mathbf{x}_j}=\varphi_j(\mathbf{x}_j)=E[Y_j|X_j=\mathbf{x}_j]=\beta_{j0}+\sum_{k=1}^{p_j} \beta_{jk}x_{jk}$. The conditional df based on this calibration function is written $F_{j}(y_j;\varphi_j(\mathbf{x}_j))$.
