...
Two applications of copula modeling in clinical trials were mentioned briefly in the introduction. Now that basic copula theory has been reviewed, we can see how these models are used in practice.
-->
...
The maximum likelihood estimates and standard errors were calculated using the `optim` function to minimize the negative log-likelihood. Table `r if (knitr::is_html_output()) '\\@ref(tab:cl-tab-html)' else '\\@ref(tab:cl-tab)'` shows parameter estimates, standard errors and odds ratios for the regression parameters. 
