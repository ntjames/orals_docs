...
```{r ex1-a, fig.cap='Contour plot of bivariate density with standard Normal and Gamma(4,1) margins combined using a Gumbel copula with $\\theta=1\\frac{1}{3}$. The small scatterplot in the top right of each shows n=500 random draws from the multivariate density', fig.show = 'hold', fig.align='center', out.width='90%'}
## Example 1a
...
```{r, eval=FALSE, fig.cap='Interactive plot for Figure \\@ref(fig:ex1-a)'}
library(rgl)
...
gc_zcol <- cut(gc_comps$z, num_col)

plotid1<-persp3d(gc_comps$x,gc_comps$y,gc_comps$z, col=col_sch[gc_zcol],
                 xlab="Normal", ylab="Gamma", zlab="")

#gc_zcol <- cut(gc_z, num_col)

#plotid1<-persp3d(y1,y2,gc_z, col=col_sch[gc_zcol],
#                 xlab="Normal", ylab="Gamma", zlab="")
...
#!contour(gc_comps$x,gc_comps$y,gc_comps$z)

# color scheme
num_col<-100
col_sch <- rev(rainbow(num_col, start = 0/6, end = 4/6))

#! need to transpose z matrix output from contour() from to plot correctly
plot_ly(x = gc_comps$x, y = gc_comps$y, z = t(gc_comps$z)) %>% 
  add_surface(colors=col_sch,
              contours = list(
                      x = list(show=FALSE, usecolormap=FALSE,
                      project=list(x=TRUE,y=FALSE,z=FALSE)),
                      y = list(show=FALSE, usecolormap=FALSE,
                      project=list(x=FALSE,y=TRUE,z=FALSE)) )) %>%
  layout(scene = list(
                 xaxis = list(title = "Normal"),
...
```{r, eval=FALSE, fig.cap='Interactive plot for Figure \\@ref(fig:ex1-b)'}
cc_z<-outer(y1,y2,FUN=function(a,b) dMvdc(matrix(c(a,b),nrow=75^2),mvd_cc))

cc_zcol <- cut(cc_z, num_col)

plotid2<-persp3d(y1,y2,cc_z, col=col_sch[cc_zcol],
                 xlab="Normal", ylab="Gamma", zlab="")
